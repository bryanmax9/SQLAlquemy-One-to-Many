...
def select_section(sess: Session) -> Section:
    """
    Select a section by the combination of the department abbreviation, course number, and section number.
    :param sess:    The connection to the database.
    ...
    while not found:
        ...
        name_count: int = sess.query(Section).filter(Section.department_abbreviation == department_abbreviation,
                                                    ...
                                                    Section.section_number == section_number).count()
        found = name_count == 1
        if not found:
            print("No section by that number in that course and department.  Try again.")
    section = sess.query(Section).filter(Section.department_abbreviation == department_abbreviation,
                                       Section.course_number == course_number,
                                       Section.section_number == section_number).first()
    return section
...
def delete_section(session: Session):
    """
    Prompt the user for a section by the department abbreviation, course number, and section number, and delete it.
    :param session: The connection to the database.
    :return:        None
    """
...
def list_sections(session: Session) -> None:
    """
    List all sections currently in the database.
    :param session: The connection to the database.
    :return: None
    """
    sections: List[Section] = list(session.query(Section).order_by(Section.course_number, Section.section_number))
